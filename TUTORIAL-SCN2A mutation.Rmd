...
d %>% filter('UniqueSample/Family' == 'Y') %>% count()
```
...
d %>% group_by(TrueRecurrence, Inheritence) %>% select(TrueRecurrence) %>% count()
```
...
```{r}
# please write your code
d %>% group_by(PatientSex, ASD) %>% count()
```
...
```{r}
# please write your code
d %>% group_by(Classification, ASD) %>% count()
```
...
```{r}
# please write your code
d %>% group_by(TrueRecurrence, Pos_hg19) %>% count()
```
