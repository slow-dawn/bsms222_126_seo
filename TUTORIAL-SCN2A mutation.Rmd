---
title: "Tutorial - SCN2A mutation in neurodevelopmental disorders"
...
```{r}
# all except for 'chr', 'AnyRecurrence' and 'TrueRecurrence'
```
...
```{r}
# write a code to do vectorization
d_P <- as.data.frame(d$p.Protein)
...
```{r}
pander(d %>% filter('UniqueSample/Family' == 'Y') %>% count())
```
...
```{r}
pander(d %>% group_by(TrueRecurrence, Inheritence) %>% count())
```
...
```{r}
d1 <- d %>% group_by(SeizureType) %>% count() %>% mutate(proportion1 = n / sum(d1$n))

pander(d1) 
```

```{r}
d2 <- d %>% group_by(Classification) %>% count() %>% mutate(proportion2 = n / sum(d2$n))
...
```{r}
pander(d %>% group_by(PatientSex, ASD) %>% count())
```
...
```{r}
pander(d %>% group_by(Classification, ASD) %>% count())
```


## 3.4 Find the position where different consequences of mutation occur
...
```{r}
pander(d %>% group_by(TrueRecurrence, Pos_hg19) %>% count())
```
