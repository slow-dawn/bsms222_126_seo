...
In each iteration $n=1, n=2$, etc…, we compute $S_n$ and store it in the $n$th entry of of s_n. 
...
```{r}
new_names <- ifelse(nchar(murders$state) > 8, c(murders$abb), NA)
```
