...
```{r}
pander(filter(murders, rate <= 0.71))
```
...
pander(filter(new_table, rate <= 0.71))
```
...
```{r}
pander(filter(murders, rank(-x) <= 5))
```
...
```{r}
pander(head(filter(murders, state %in% c("New York", "Texas"))))
```
...
```{r}
filter(murders, population < 5000000 & region == "Northeast") %>% head()
```
...
```{r}
pander(select(my_states, state, 'rank(-x)'))
```
...
```{r}
a <- mutate(murders, rate =  total / population * 100000, 
       rank = rank(-rate)) %>%
  select(state, rate, rank)
```
```{r}
...
my_states <- murders %>%
  mutate(rate, rank) %>%
  filter(murder_rate < 1 & region %in% c("Northeast", "West")) %>%
  select(state, rate, rank)
```
